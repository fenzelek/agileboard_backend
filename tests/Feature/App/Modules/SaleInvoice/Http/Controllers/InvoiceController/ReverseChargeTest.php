...
class ReverseChargeTest extends BrowserKitTestCase
{
    ...
    /** @test */
    public function store_validation_error_invoice_reverse_charge()
    {
        list($company, $delivery_address, $incoming_data) = $this->setFinancialEnvironment();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, true);
        $incoming_data['invoice_type_id'] = InvoiceType::findBySlug(InvoiceTypeStatus::REVERSE_CHARGE)->id;
    ...
    /** @test */
    public function store_validation_error_fake_invoice_reverse_charge()
    {
        list($company, $delivery_address, $incoming_data) = $this->setFinancialEnvironment();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, true);
        $incoming_data['invoice_type_id'] = InvoiceType::findBySlug(InvoiceTypeStatus::REVERSE_CHARGE)->id;
        $invoice_reverse_charge = InvoiceReverseCharge::findBySlug(InvoiceReverseChargeType::OUT);
        $invoice_reverse_charge_id = $invoice_reverse_charge->id;
        $invoice_reverse_charge->delete();
        $incoming_data['invoice_margin_procedure_id'] = $invoice_reverse_charge_id;
        $this->post('invoices?selected_company_id=' . $company->id, $incoming_data)
            ->assertResponseStatus(422);
    ...
    /** @test */
    public function store_disabled_reverse_charge_module_error()
    {
        list($company, $delivery_address, $incoming_data) = $this->setFinancialEnvironment();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, false);
        $incoming_data['invoice_type_id'] = InvoiceType::findBySlug(InvoiceTypeStatus::REVERSE_CHARGE)->id;
        $this->post('invoices?selected_company_id=' . $company->id, $incoming_data)
            ->assertResponseStatus(422);
    ...
    {
        list($company, $delivery_address, $incoming_data) = $this->setFinancialEnvironment();
        $incoming_data['invoice_type_id'] = InvoiceType::findBySlug(InvoiceTypeStatus::REVERSE_CHARGE_CORRECTION)->id;
        $this->post('invoices?selected_company_id=' . $company->id, $incoming_data);
    ...
    /** @test */
    public function store_it_returns_validation_error_corrected_reverse_charge_invoice()
    {
        $company = $this->login_user_and_return_company_with_his_employee_role();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, true);
        $invoice = factory(Invoice::class)->create([
    ...
    /** @test */
    public function store_validation_error_correction_type_for_reverse_charge_correction()
    {
    ...
    /** @test */
    public function store_disabled_margin_module_error_try_issuing_reverse_charge_correction()
    {
        list($company, $invoice, $invoice_items, $incoming_data) = $this->setFinancialEnvironmentForCorrection();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, false);
        $incoming_data['invoice_type_id'] = InvoiceType::findBySlug(InvoiceTypeStatus::REVERSE_CHARGE_CORRECTION)->id;
    ...
    public function store_invoice_number_queue_continue_vat_queue()
    {
        ...
        $incoming_data['invoice_type_id'] = InvoiceType::findBySlug(InvoiceTypeStatus::REVERSE_CHARGE)->id;
        $incoming_data['invoice_reverse_charge_id'] = InvoiceReverseCharge::findBySlug(InvoiceReverseChargeType::OUT)->id;
        $incoming_data['vat_sum'] = 0;
        list($incoming_data, $vat_rate) = $this->setNpVatRate($incoming_data);
    ...
    /** @test */
    public function store_invoice_reverse_charge_correction_number_queue_continue_correction_queue()
    {
        list($company, $invoice, $invoice_items, $incoming_data) = $this->setFinancialEnvironmentForCorrection();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, true);
        $invoice_vat = factory(Invoice::class)->create([
    ...
    public function store_correct_data_in_database()
    {
        ...
        $incoming_data['invoice_reverse_charge_id'] = InvoiceReverseCharge::findBySlug(InvoiceReverseChargeType::OUT)->id;
        $incoming_data['vat_sum'] = 0;
        list($incoming_data, $vat_rate) = $this->setNpVatRate($incoming_data, $vat_rate = null);
        $this->post('invoices?selected_company_id=' . $company->id, $incoming_data);
        ...
        $this->assertNull($invoice_margin->invoice_margin_procedure_id);
        $this->assertSame($incoming_data['invoice_reverse_charge_id'], $invoice_margin->invoice_reverse_charge_id);
    ...
    /** @test */
    public function store_correct_data_in_database_for_reverse_charge_correction()
    {
        list($company, $invoice, $invoice_items, $incoming_data) = $this->setFinancialEnvironmentForCorrection();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, true);
        ...
        $this->assertNull($correction_margin->invoice_margin_procedure_id);
        $this->assertSame($incoming_data['invoice_reverse_charge_id'], $correction_margin->invoice_reverse_charge_id);
    ...
    public function store_response_correct()
    {
        ...
        $this->assertSame($incoming_data['invoice_reverse_charge_id'], $json_data['invoice_reverse_charge_id']);
    }

    /** @test */
    public function update_vat_sums_and_reverse_charge_id_validation_error()
    {
    ...
    /** @test */
    public function update_vat_sums_and_reverse_charge_id_and_type_validation_error()
    {
        list($company, $payment_method, $contractor, $invoice, $items_data, $taxes_data) = $this->setFinancialEnvironmentForUpdate();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, true);
        $invoice->update([
        ...
        $invoice_reverse_charge = InvoiceReverseCharge::findBySlug(InvoiceReverseChargeType::OUT_EU_CUSTOMER_TAX);
        $invoice_reverse_charge_id = $invoice_reverse_charge->id;
        $invoice_reverse_charge->delete();
        $this->put('invoices/' . $invoice->id . '?selected_company_id=' . $company->id, [
            'invoice_type_id' => InvoiceType::findBySlug(InvoiceTypeStatus::VAT)->id,
            'invoice_reverse_charge_id' => $invoice_reverse_charge_id,
            'vat_sum' => 15.78,
            'payment_method_id' => $payment_method->id,
            'items' => $items_data,
            'taxes' => $taxes_data,
    ...
    public function update_data_in_database()
    {
        list($company, $payment_method, $contractor, $invoice, $items_data, $taxes_data) = $this->setFinancialEnvironmentForUpdate();
        $this->setAppSettings($company, ModuleType::INVOICES_REVERSE_CHARGE_ENABLED, true);
        $invoice_type_margin = InvoiceType::findBySlug(InvoiceTypeStatus::REVERSE_CHARGE);
        $invoice->update([
        ...
        $this->assertSame($invoice_reverse_charge->id, $invoice_fresh->invoice_reverse_charge_id);
    ...
    public function update_check_response_json()
    {
        ...
        $this->assertSame($invoice_reverse_charge->id, $response['invoice_reverse_charge_id']);
    }
     ...
     * @return mixed
     */
    public function setNpVatRate($incoming_data)
    {
