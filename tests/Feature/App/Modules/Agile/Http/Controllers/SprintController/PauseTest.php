 ...
 * Class PauseTest.
 */
class PauseTest extends BrowserKitTestCase
{
    use DatabaseTransactions, ProjectHelper;

    /**
     * @test
     * @covers \App\Modules\Agile\Http\Controllers\SprintController::pause
     */
    public function success_response()
    {
        ...
        $this->put('/projects/' . $project->id . '/sprints/' . $sprint->id .
            '/pause?selected_company_id=' . $company->id, [])
            ->seeStatusCode(200);

        Event::assertDispatched(PauseSprintEvent::class, function ($e) use ($project, $sprint) {
            if (
        ...
        $this->assertSame(Sprint::PAUSED, $response_sprint['status']);
    ...
    public function success_db()
    {
        ...
        $this->assertSame(Sprint::PAUSED, $sprint->status);
        $this->assertSame($now->toDateTimeString(), $sprint->updated_at->toDateTimeString());
        $this->assertSame($now->toDateTimeString(), $sprint->paused_at->toDateTimeString());
    }
    ...
    public function error_sprint_not_exist()
    {
        ...
        $this->put('/projects/' . $project->id . '/sprints/0/pause?selected_company_id=' .
            $company->id, []);
