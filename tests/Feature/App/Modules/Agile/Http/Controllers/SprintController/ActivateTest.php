...
class ActivateTest extends BrowserKitTestCase
{
    use DatabaseTransactions, ProjectHelper;

    /**
     * @test
     * @covers \App\Modules\Agile\Http\Controllers\SprintController::activate
     */
    public function success_response()
    {
        ...
        $this->put('/projects/' . $project->id . '/sprints/' . $sprint->id .
            '/activate?selected_company_id=' . $company->id, [])
            ->seeStatusCode(200);

        Event::assertDispatched(ActiveSprintEvent::class, function ($e) use ($project, $sprint) {
            if (
    ...
    public function success_db()
    {
        ...
        $this->assertSame($now->toDateTimeString(), $sprint->activated_at->toDateTimeString());
    }
    ...
    public function error_sprint_not_exist()
    {
        ...
        $this->put('/projects/' . $project->id . '/sprints/0/activate?selected_company_id=' .
            $company->id, []);
