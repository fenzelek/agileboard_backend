...
trait InvolvedServiceTrait
{
    private function getInvolvedRequestMock(int $company_id, int $project_id, array $involved_ids): IInvolvedRequest
    {
        ...
        $involved_request->allows('getSelectedCompanyId')->andReturns($company_id);
        $involved_request->allows('getProjectId')->andReturns($project_id);
        $involved_request->allows('getInvolvedIds')->andReturns($involved_ids);
