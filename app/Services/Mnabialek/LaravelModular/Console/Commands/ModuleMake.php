...
namespace App\Services\Mnabialek\LaravelModular\Console\Commands;

use App\Services\Mnabialek\LaravelModular\Console\Traits\ModuleCreator;
use App\Services\Mnabialek\LaravelModular\Models\Module;
use App\Services\Mnabialek\LaravelModular\Traits\Replacer;
    ...
    protected function addModuleToConfigurationFile(Module $module)
    {
        ...
        $result =
            preg_replace_callback(
                $this->laravel['modular.config']->autoAddPattern(),
                function ($matches) use ($module, $configFile) {
                    return $matches[1] . $matches[2] .
                    $this->replace(
                        $this->laravel['modular.config']->autoAddTemplate(),
                        $module
                    ) .
                    $matches[3];
                },
